...
/usr/bin/rsync --dry-run -aI --progress --recursive --exclude='upload_xls' --exclude='.git' --exclude='logs' --exclude='test/local_config.py' 本地文件夹 admin@IP地址:/data/sites/genesis_backend/

/usr/bin/rsync -aI --progress --recursive --exclude='upload_xls' --exclude='.git' --exclude='logs' --exclude='test/local_config.py' 本地文件夹 admin@IP地址:/data/sites/genesis_backend/

-a              归档，保留文件的原有属性，相当于-rlptgoD
-v              详细输出模式
--dry-run       显示哪些文件将被传输
--progress      显示进度信息
--recursive     递归到目录中
--exclude       指定排除规则来排除不需要传输的文件(可用正则表达式)
```
